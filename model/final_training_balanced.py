...
if __name__ == "__main__":
    ...
    with open(path + "new_source_10_shrink.pickle", "rb") as f: #source2 actuaL: new_source # _23 # new_source_from_lower
        source = pickle.load(f)
    with open(path + "new_target_10_shrink.pickle", "rb") as f: #target_NN2, acutal:new_target # _23 # new_target_from_lower.pickle
        target = pickle.load(f)
    with open(path + "vocabulary_10_shrink.pickle", "rb") as f: # _23 # final_lower # vocabulary_from_lower
    ...
    #criterion_gen_coop = torch.nn.CrossEntropyLoss() # loss for cooperative
    #criterion_disc_coop = torch.nn.CrossEntropyLoss()

    #citerion_g_hat = torch.nn.CrossEntropyLoss()
    #citerion_d_hat = torch.nn.CrossEntropyLoss()
    # sonst 6380
    total_inst = 5612 #5314
    ...
    channel_1_crit = torch.nn.CrossEntropyLoss()
    channel_2_crit = torch.nn.CrossEntropyLoss()

    # dataset stuff
    torch.autograd.set_detect_anomaly(True)

    #weights = weight.double()
    #sampler = WeightedRandomSampler(torch.DoubleTensor(weights), int(sum(class_counts)))
    ...
    for epoch in range(EPOCHS):
        ...
        for (batch, (inp, targ)) in enumerate(dataset):
            ...
            # noise = torch.random # noise as GAN input
            # xs, xy, lens = sort_batch(inp, targ, inp_len)
            ...
            #loss_g_hat = channel_2_crit(res_fake, targ)
            #"""

            final_res = disc_1(out)  # channel 1 isn't really a discriminator like in the usual setting of GAN
    ...
    torch.save(gen_1.state_dict(), path+"gen_1_balanced")
    torch.save(disc_1.state_dict(), path+"disc_1_balanced")
    torch.save(gen_2.state_dict(), path+"gen_2_balanced")
    torch.save(disc_2.state_dict(), path+"disc2_balanced")
